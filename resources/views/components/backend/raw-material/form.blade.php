                                ...
                                <div class="pt-4">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" id="title" name="title" class="form-control"
                                               value="{{ old('title', $rawMaterial->title ?? '') }}">
                                        @error('title')
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea id="description"
                                                  ...
                                                  rows="10">
                                                {{ old('description', $rawMaterial->description ?? '') }}
                                            </textarea>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-md-4">
                <div class="card shadow">
                    <div class="card-body">
                            ...
                            <div class="form-group" style="margin: 0;">
                                ...
                                <span class="input-group-btn">
                                    ...
                                    <input id="thumbnail_1" class="form-control" type="text" name="filepath"
                                           value="{{ $rawMaterial->file ? asset('storage/' . $rawMaterial->file->path) : '' }}"
                                           readonly>
                                ...
                                @if ($rawMaterial->file)
                                    <div id="holder_1" style="margin-top:15px; max-height:100px;">
                                        <img src="{{ asset('storage/' . $rawMaterial->file->path) }}"
                                             style="height: 80px;">
                                    </div>
                                @else
                        ...
                        <div class="mb-3 mt-3">
                            ...
                            <select class="form-control" name="category_id">
                                ...
                                @foreach($categories as $id => $name)
                                    <option value="{{ $id }}"
                                        {{ old('category_id', $rawMaterial->category_id ?? '') == $id ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3 mt-3">
                            <label for="warehouse_id">Хомашё омбори</label>
                            <select name="warehouse_id" id="warehouse_id" class="form-control" required>
                                <option value="">Хомашё омборини танланг</option>
                                @foreach($warehouses as $id => $title)
                                    <option value="{{ $id }}"
                                        {{ old('warehouse_id', $rawMaterial->warehouse_id ?? '') == $id ? 'selected' : '' }}>
                                        {{ $title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" id="status" class="form-control">
                                @foreach (\App\Services\StatusService::getList() as $key => $label)
                                    <option
                                        value="{{ $key }}" {{ old('status', $rawMaterial->status ?? '') == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if (Route::currentRouteName() == 'raw-material.create')
                            <button type="submit" class="btn btn-success">{{ 'Сақлаш' }}</button>
                        @elseif (Route::currentRouteName() == 'raw-material.edit')
