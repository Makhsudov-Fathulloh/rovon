...
<div class="container-fluid">
    <div class="excel-theme">
        ...
        <div class="shift-tabs">
            ...
            <span class="shift-link active"> {{ $shiftOutput->shift->title }} </span>
            {{-- @foreach($shifts as $s)
        ...
        <form action="{{ $action ?? '#' }}" method="POST">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text"
                                    ...
                                    value="{{ $oldInputs[$worker->id][$stage->id]['stage_count'] ?? '' }}"
                                    placeholder="-">
                            </td>
                            <td>
                                <input type="text" step="0.001"
                                    name="input[{{ $worker->id }}][{{ $stage->id }}][defect_amount]"
                                    class="cell-input defect-input defect-input-cell filter-numeric-decimal"
                                    value="{{ $workerDefects[$worker->id] ?? '' }}"
                                    placeholder="-">
                            </td>
                        </tr>
                        @endforeach
            ...
            <div class="footer-actions">
                <button type="submit" class="save-btn">САҚЛАШ</button>
            </div>
        </form>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function calculateTotals() {
            ...
            document.querySelectorAll('.qty-input').forEach(input => {
                stageSum += getNumericValue(input);
            });

            document.getElementById('total-stage-{{ $stage->id }}')
                .innerText = stageSum.toLocaleString('ru-RU');
