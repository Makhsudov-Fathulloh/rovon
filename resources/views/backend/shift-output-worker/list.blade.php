                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            @forelse($workers as $worker)
                                <tr class="text-center" id="row-{{ $worker->id }}">
                                    ...
                                    <td>{{ optional($worker->user)->username }}</td>
                                    <td>{{ optional($worker->shiftOutput->shift)->title }}</td>
                                    <td>{{ optional($worker->shiftOutput->stage)->title }}</td>
                                    <td class="text-success fw-bold">{{ number_format( $worker->stage_count , 0, '', ' ') }}</td>
                                    <td class="text-danger fw-bold">{{ $worker->defect_amount }} кг</td>
                                    <td class="text-info fw-bold">{{ number_format( $worker->price, 0, '', ' ') }}</td>
                    ...
                    <div class="d-md-none">
                        <div class="d-flex mb-2">
                            <input type="text" name="filters[title]" value="{{ request('filters.title') }}"
                                   ...
                                   placeholder="Смена махсулот номини киритинг">
                        ...
                        @forelse($workers as $worker)
                            <div class="card border">
                                <div class="card-body">
                                    <p>
                                        <strong>{!! sortLink('id', 'ID:') !!}</strong>{{ $worker->id }} </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('user_id', 'Ходим:') !!}</strong>{{ optional($worker->user)->username }} </p>
                                    <p>
                                        <strong>{!! sortLink('shift_output_id', 'Смена:') !!}</strong>{{ optional($worker->shiftOutput->shift)->title }} </p>
                                    <p>
                                        <strong>{!! sortLink('stage_id', 'Махсулот:') !!}</strong>{{ optional($worker->shiftOutput->stage)->title }} </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('stage_count', 'Микдори:') !!}</strong><span class="text-success fw-bold">{{ number_format($worker->stage_count, 0, '', ' ') }} та</span></p>
                                    <p class="card-text">
                ...
                @foreach($userStatistics as $statistics)
                    <div class="row text-center">
                        ...
                        <div class="col-md-3 alert alert-success">
                            ...
                            Нархи: <strong class="h4">{{ number_format($statistics['monthly_price'], 0, '', ' ') }}</strong> та
                        </div>
                        <div class="col-md-3 alert alert-secondary">
                            ...
                            Нархи: <strong class="h4">{{ number_format($statistics['yearly_price'], 0, '', ' ') }}</strong> та
                        </div>
                    </div>
                @endforeach
