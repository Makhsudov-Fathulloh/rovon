            ...
            <div class="table-responsive card-body">
                <form id="rawMaterialVariationFilterForm" method="GET"
                    ...
                    <div class="table-responsive d-none d-md-block">
                        <table class="table table-bordered table-hover">
                            ...
                            <tr class="text-center">
                                ...
                                <th>{!! sortLink('raw_material_id', 'Хомашё тури') !!}</th>
                            ...
                            <tr>
                                ...
                                <th>
                                    <select name="filters[raw_material_id]"
                                            class="form-control form-control-sm filter-select2 w-100">
                                        <option value="">Барчаси</option>
                                        @foreach($rawMaterials as $id => $title)
                                            <option
                                                value="{{ $id }}" {{ request('filters.raw_material_id') == $id ? 'selected' : '' }}>{{ $title }}</option>
                                        @endforeach
                                    </select>
                                </th>
                            ...
                            @forelse($rawMaterialVariations as $rawMaterialVariation)
                                <tr class="text-center" id="row-desktop-{{ $rawMaterialVariation->id }}">
                                    ...
                                    <td>{{ $rawMaterialVariation->rawMaterial->title }}</td>
                                    <td>{{ $rawMaterialVariation->title }}</td>
                                    <td>
                                    ...
                                    <td class="price fw-bold text-success">
                                        {{ PriceHelper::format($rawMaterialVariation->price, $rawMaterialVariation->currency) }}
                                    </td>
                                    <td class="count fw-bold text-primary">
                                        {{ CountHelper::format($rawMaterialVariation->count, $rawMaterialVariation->unit) }}
                                    </td>
                    ...
                    <div class="d-md-none">
                        ...
                        @forelse($rawMaterialVariations as $rawMaterialVariation)
                            <div class="card border" id="row-mobile-{{ $rawMaterialVariation->id }}">
                                <div class="card-body">
                                    ...
                                    <p class="card-text ">
                                        <strong>{!! sortLink('id', 'ID:') !!} </strong>{{ $rawMaterialVariation->id }}
                                    </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('raw_material_id', 'Хомашё тури:') !!} </strong>{{ optional($rawMaterialVariation->rawMaterial)->title }}
                                    </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('code', 'Код:') !!} </strong>{{ $rawMaterialVariation->code }}
                                    </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('title', 'Номи:') !!} </strong>{{ $rawMaterialVariation->title }}
                                    </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('price', 'Нархи:') !!}</strong>
                                        <span
                                            class="price fw-bold text-success">{{ PriceHelper::format($rawMaterialVariation->price, $rawMaterialVariation->currency, false) }}</span> {{ StatusService::getCurrency()[$rawMaterialVariation->currency] }}
                                    </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('count', 'Микдори:') !!}</strong>
                                        <span
                                            class="count fw-bold text-primary">{{ CountHelper::format($rawMaterialVariation->count, $rawMaterialVariation->unit, false) }}</span> {{ StatusService::getTypeCount()[$rawMaterialVariation->unit] }}
                                    </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('total_price', 'Умумий:') !!}</strong>
                                        <span
                                            class="total_price fw-bold text-info">{{ PriceHelper::format($rawMaterialVariation->total_price, $rawMaterialVariation->currency, false) }}</span> {{ StatusService::getCurrency()[$rawMaterialVariation->currency] }}
                                    </p>
                                    <p class="card-text">
                                        <strong>{!! sortLink('status', 'Статус:')  !!} </strong> {{ StatusService::getList()[$rawMaterialVariation->status] ?? '-' }}
                                    </p>
